...
@available(iOS 9.0, *)
extension UIView {
    func addSubviewWithConstraints(fillingCompletely subView: UIView) {
        subView.translatesAutoresizingMaskIntoConstraints = false
        addSubview(subView)
        subView.addConstraints(fillingSuperViewCompletely: self)
    }
    
    func addConstraints(fillingSuperViewCompletely superView: UIView) {
...
extension UIView {
    ...
    @discardableResult
    func blur(style: UIBlurEffectStyle = .regular, alpha: CGFloat = 0.35) -> UIVisualEffectView {
