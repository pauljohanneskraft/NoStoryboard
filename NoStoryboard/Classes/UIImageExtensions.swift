...
extension UIImage {
    static let serialQueue = DispatchQueue(label: "\(UIImage.self)")
...
extension UIImage {
    ...
    static func fetch(from url: URL, completionHandler: @escaping (URL, UIImage) -> ()) {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            let downloadThumbnailTask = URLSession(configuration: .default).dataTask(with: url) { (data, response, error) in
                ...
                if let imageData = data, let thumbnail = UIImage(data: imageData) {
                    UIImage.cache.setObject(thumbnail, forKey: url as NSURL)
...
extension UIImage {
    ...
    func scaledAndCropped(for rect: CGRect) -> UIImage {
        return UIImage.serialQueue.sync {
